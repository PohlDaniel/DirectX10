...
Model::Model() : pVertexBuffer(0), pIndexBuffer(0), m_size(0), m_stride(0), m_offset(0), m_numberOfBytes(0){
...
void Model::setRotPos(const D3DXVECTOR3 &axis, float degrees, float dx, float dy, float dz){

	m_modelMatrix->setRotPos(axis, degrees, dx, dy, dz);
}

void Model::setRotXYZPos(const D3DXVECTOR3 &axisX, float degreesX,
...
void Model::rotate(const D3DXVECTOR3 &axis, float degrees){
...
const D3DXMATRIXA16 &Model::getTransformationMatrix() const{

	return m_modelMatrix->getTransformationMatrix();
}

const D3DXMATRIXA16 &Model::getInvTransformationMatrix() const{

	return m_modelMatrix->getInvTransformationMatrix();

}

const D3DXVECTOR3 &Model::getCenter() const{
...
void Model::createBuffer(ID3D10Device1* pD3DDevice){
...
bool Model::loadObject(const char* a_filename, D3DXVECTOR3 &rotate, float degree, D3DXVECTOR3& translate, float scale){
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
Mesh::Mesh(std::string mltName, int numberTriangles, Model* model) : pVertexBuffer(0), pIndexBuffer(0), m_size(0), m_stride(0), m_offset(0), m_numberOfBytes(0){
...
Mesh::Mesh(int numberTriangles, Model* model) : pVertexBuffer(0), pIndexBuffer(0), m_size(0), m_stride(0), m_offset(0), m_numberOfBytes(0){
...
void Mesh::createBuffer(ID3D10Device1* pD3DDevice){
...
void Mesh::loadTextureFromMlt(ID3D10Device1* pD3DDevice){
	//load textures
	if (m_material.diffuseTexPath != ""){	

		if (FAILED(D3DX10CreateShaderResourceViewFromFile(pD3DDevice, (m_model->getModelDirectory() + "/" + getMaterial().diffuseTexPath).c_str(), NULL, NULL, &m_pDiffuseTex, NULL)))
			std::cout << "Could not load diffuse texture!" << std::endl;

	}

	if (m_material.bumpMapPath != ""){
		if (FAILED(D3DX10CreateShaderResourceViewFromFile(pD3DDevice, (m_model->getModelDirectory() + "/" + getMaterial().bumpMapPath).c_str(), NULL, NULL, &m_pNormalMap, NULL)))
			std::cout << "Could not load normalMap texture!" << std::endl;
	}

	if (m_material.displacementMapPath != ""){
		if (FAILED(D3DX10CreateShaderResourceViewFromFile(pD3DDevice, (m_model->getModelDirectory() + "/" + getMaterial().displacementMapPath).c_str(), NULL, NULL, &m_pDisplacementMap, NULL)))
			std::cout << "Could not load displacementMap texture!" << std::endl;
	}
