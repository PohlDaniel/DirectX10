...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPTSTR lpCmdLine, int nCmdShow){
	...
	D3DXVECTOR3 camPos(0.0f, 0.0f, 25.0f);
	...
	//skybox = new SkyBox("../skyboxes/hw_morning", 500, true, false, g_pD3DDevice);
	//skybox = new SkyBox("../skyboxes/sea", 500, true, true, g_pD3DDevice);
	/////////////////////////////Model//////////////////////////////////////////////


	model = new Model();
	model->loadObject("objs/cube/cube.obj");
	...
	shaderFX = new ShaderFX("skybox.fx", g_pD3DDevice);
	...
	material = { { 0.3f, 0.3f, 0.3f }, { 0.7f, 0.7f, 0.7f }, { 1.0f, 1.0f, 1.0f }, 90.0f };
	shaderFX->loadMaterial("material", material);
...
void render(){
	...
	shaderFX->setMatrix("modelMatrix", model->getTransformationMatrix());
	shaderFX->setMatrix("viewMatrix", camera->getViewMatrix());
	shaderFX->setMatrix("projectionMatrix", camera->getProjectionMatrix());
	shaderFX->setMatrix("normalMatrix", Matrix::getNormalMatrix(model->getTransformationMatrix() * camera->getViewMatrix()));

	shaderFX->setFloatVector("lightPos", camera->getPosition(), sizeof(D3DXVECTOR3));

	shaderFX->setFloatVectorInStruct("light", "pos", camera->getPosition(), sizeof(D3DXVECTOR3));
	shaderFX->setFloatVectorInStruct("light", "ambient", D3DXVECTOR3(1.0f, 1.0f, 1.0f), sizeof(D3DXVECTOR3));
	shaderFX->setFloatVectorInStruct("light", "diffuse", D3DXVECTOR3(1.0f, 1.0f, 1.0f), sizeof(D3DXVECTOR3));
	shaderFX->setFloatVectorInStruct("light", "specular", D3DXVECTOR3(1.0f, 1.0f, 1.0f), sizeof(D3DXVECTOR3));
	...
	for (UINT p = 0; p < shaderFX->m_techDesc.Passes; p++){
		...
		if (skybox->m_cubeMapTex){
			...
			shaderFX->setTexture("cubeMapTexture", skybox->m_cubeMapTex);
		}
	...
	/*for (UINT p = 0; p < shaderFX->m_techDesc.Passes; p++){
		for (int j = 0; j < model->mesh.size(); j++){
			...
			if (g_shaderResourceView){
				...
				shaderFX->setTexture("colorMapTexture", g_shaderResourceView);
			}

			if (model->mesh[j]->m_pDiffuseTex){
				shaderFX->setTexture("colorMapTexture", model->mesh[j]->m_pDiffuseTex);
			}

			if (model->mesh[j]->m_pNormalMap){
				shaderFX->setTexture("normalMapTexture", model->mesh[j]->m_pNormalMap);
			}

			if (model->mesh[j]->m_pDisplacementMap){
				shaderFX->setTexture("displacementMapTexture", model->mesh[j]->m_pDisplacementMap);
			}
			//apply technique
			shaderFX->m_pTechnique->GetPassByIndex(p)->Apply(0);
