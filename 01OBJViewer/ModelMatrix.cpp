...
void ModelMatrix::setRotXYZPos(const D3DXVECTOR3 &axisX, float degreesX,
	...
	if (degreesX != 0 || (!axisX[0] == 0.0 && !axisX[1] == 0.0 && !axisX[2] == 0.0)){
		...
		D3DXMatrixRotationAxis(&rotMtx, &axisX, degreesX);
	...
	if (degreesY != 0 || (!axisY[0] == 0.0 && !axisY[1] == 0.0 && !axisY[2] == 0.0)){
		...
		D3DXMatrixRotationAxis(&rotMtx, &axisY, degreesY);
		D3DXMatrixTranspose(&invRotMtx, &rotMtx);
	...
	if (degreesZ != 0 || (!axisZ[0] == 0.0 && !axisZ[1] == 0.0 && !axisZ[2] == 0.0)){
		...
		D3DXMatrixRotationAxis(&rotMtx, &axisZ, degreesZ);
		D3DXMatrixTranspose(&invRotMtx, &rotMtx);
	...
	//T = T * Translate
	T._11 = T._11 + T._14 * dx; T._12 = T._12 + T._14 * dz; T._13 = T._13 + T._14 * dy;
	T._21 = T._21 + T._24 * dx; T._22 = T._22 + T._24 * dz; T._23 = T._23 + T._24 * dy;
	T._31 = T._31 + T._34 * dx; T._32 = T._32 + T._34 * dz; T._33 = T._33 + T._34 * dy;
	T._41 = T._41 + T._44 * dx; T._42 = T._42 + T._44 * dz; T._43 = T._43 + T._44 * dy;

	//T^-1 = Translate^-1 * T^-1 
	invT._41 = invT._41 - dx*invT._11 - dz*invT._21 - dy*invT._31;
	invT._42 = invT._42 - dx*invT._12 - dz*invT._22 - dy*invT._32;
	invT._43 = invT._43 - dx*invT._13 - dz*invT._23 - dy*invT._33;
	invT._44 = invT._44 - dx*invT._14 - dz*invT._24 - dy*invT._34;
...
void ModelMatrix::translate(float dx, float dy, float dz){
	...
	T._31 = T._31 + T._34 * dx; T._32 = T._32 + T._34 * dz; T._33 = T._33 + T._34 * dy;
	T._41 = T._41 + T._44 * dx; T._42 = T._42 + T._44 * dz; T._43 = T._43 + T._44 * dy;*/
	D3DXMatrixTranslation(&translate, dx, dz, dy);
	T = T * translate;
...
void ModelMatrix::scale(float a, float b, float c){
	...
	// T = T * scale
	T._11 = T._11 * a;  T._12 = T._12 * b; T._13 = T._13 * b;
	...
	// T^-1 =  Scale^{-1} * T^-1 
	invT._11 = invT._11 * (1.0 / a); invT._12 = invT._12 * (1.0 / a); invT._13 = invT._13 * (1.0 / a); invT._14 = invT._14 * (1.0 / a);
	invT._21 = invT._21 * (1.0 / b); invT._22 = invT._22 * (1.0 / b); invT._23 = invT._23 * (1.0 / b); invT._24 = invT._24 * (1.0 / b);
	invT._31 = invT._31 * (1.0 / c); invT._32 = invT._32 * (1.0 / c); invT._33 = invT._33 * (1.0 / c); invT._34 = invT._34 * (1.0 / c);
